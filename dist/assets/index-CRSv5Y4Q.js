(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const a of s.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&o(a)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();function P(){const e=document.createElement("nav");return e.className="nav container flex items-center justify-between",e.innerHTML=`
  ...
  `,e}const H="modulepreload",M=function(e){return"/111111111111111111111111111111/"+e},C={},S=function(n,t,o){let r=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),i=(a==null?void 0:a.nonce)||(a==null?void 0:a.getAttribute("nonce"));r=Promise.allSettled(t.map(l=>{if(l=M(l),l in C)return;C[l]=!0;const p=l.endsWith(".css"),h=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${h}`))return;const d=document.createElement("link");if(d.rel=p?"stylesheet":H,p||(d.as="script"),d.crossOrigin="",d.href=l,i&&d.setAttribute("nonce",i),document.head.appendChild(d),p)return new Promise((w,E)=>{d.addEventListener("load",w),d.addEventListener("error",()=>E(new Error(`Unable to preload CSS for ${l}`)))})}))}function s(a){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=a,window.dispatchEvent(i),!i.defaultPrevented)throw a}return r.then(a=>{for(const i of a||[])i.status==="rejected"&&s(i.reason);return n().catch(s)})},x={get(e,n=null){try{const t=localStorage.getItem(e);return t?JSON.parse(t):n}catch{return n}},set(e,n){try{localStorage.setItem(e,JSON.stringify(n))}catch{}}};function A(e,n=300){let t;return(...o)=>{clearTimeout(t),t=setTimeout(()=>e(...o),n)}}function L(){const e=document.createElement("div");e.className="container",e.innerHTML=`
    <div class="card">
  ...
  `;const n=e.querySelector("#list"),t=e.querySelector("#loading"),o=e.querySelector("#search"),r=e.querySelector("#onlyFav"),s=e.querySelector("#episodes");let a=1,i=!1,l=!1,p="";function h(u){const c=x.get("rm-favs",[]);u.forEach(f=>n.appendChild(q(f,c)))}async function d(){if(!(i||l)){i=!0,t.textContent="Cargando...";try{const u=`${y}/character?page=${a}&name=${encodeURIComponent(p)}`,c=await fetch(u);if(!c.ok){l=!0,t.textContent="No hay más.";return}const f=await c.json();h(f.results),a++,f.info.next||(l=!0),t.textContent=""}catch(u){console.error(u),t.textContent="Error cargando"}i=!1}}window.addEventListener("scroll",()=>{window.innerHeight+window.scrollY>=document.body.offsetHeight-300&&d()});const w=A(()=>{n.innerHTML="",a=1,l=!1,p=o.value.trim(),d()},400);o.addEventListener("input",w),r.addEventListener("change",()=>{const u=x.get("rm-favs",[]);if(r.checked){if(n.innerHTML="",u.length===0){t.textContent="No hay favoritos";return}t.textContent="Cargando favoritos...",Promise.all(u.map(c=>fetch(`${y}/character/${c}`).then(f=>f.json()))).then(c=>{n.innerHTML="",h(c),t.textContent=""}).catch(c=>{t.textContent="Error",console.error(c)})}else n.innerHTML="",a=1,l=!1,d()});async function E(){let u=1,c=!0;for(;c;){const f=await fetch(`${y}/episode?page=${u}`);if(!f.ok)break;const v=await f.json();v.results.forEach(m=>{const g=document.createElement("option");g.value=m.id,g.textContent=`${m.id} - ${m.name}`,s.appendChild(g)}),v.info.next?u++:c=!1}}return s.addEventListener("change",async()=>{const u=s.value;if(!u){n.innerHTML="",a=1,l=!1,d();return}t.textContent="Cargando episodio...";try{const v=(await(await fetch(`${y}/episode/${u}`)).json()).characters.map(m=>m.split("/").pop());n.innerHTML="";for(let m=0;m<v.length;m+=20){const g=v.slice(m,m+20),b=await(await fetch(`${y}/character/${g.join(",")}`)).json(),T=Array.isArray(b)?b:[b];h(T)}t.textContent=""}catch(c){t.textContent="Error",console.error(c)}}),E(),d(),e}function I(){const n=new URLSearchParams(location.hash.split("?")[1]).get("id"),t=document.createElement("div");return t.className="container",t.innerHTML='<div class="card">Cargando...</div>',fetch(`https://rickandmortyapi.com/api/character/${n}`).then(o=>o.json()).then(o=>{t.innerHTML=`<div class="card">
    ...
    </div>`}).catch(o=>{t.innerHTML='<div class="card">Error cargando personaje</div>',console.error(o)}),t}const N={"":L,"#/":L,"#/catalogo":j,"#/detalle":I};function O(){const e=document.getElementById("view");function n(){const t=location.hash.split("?")[0]||"#/",o=N[t]||L;e.innerHTML="";const r=o();e.appendChild(r)}window.addEventListener("hashchange",n),location.hash||(location.hash="#/"),n()}const $=document.getElementById("app");window.addEventListener("error",e=>{$.insertAdjacentHTML("beforeend",`<pre style="color:red">JS error: ${e.message}</pre>`)});$.innerHTML=`
  <div id="root">
...
`;try{const e=P();document.getElementById("nav").appendChild(e),O()}catch(e){console.error(e),$.insertAdjacentHTML("beforeend",`<pre style="color:red">Init error: ${e.message}</pre>`)}
